  ...
  const existingBacklinksNodeIndex = tree.children.findIndex(
    (node: UNIST.Node): node is MDAST.Heading =>
      is(node, {
        type: 'heading',
        depth: 2,
      }) && is((node as MDAST.Heading).children[0], { value: 'Referred in' })
  )

  if (existingBacklinksNodeIndex === -1) {
    const insertionPoint =
      tree.children.find((node) => is(node, isClosingMatterNode)) ||
      tree.children.find((node: UNIST.Node): node is MDAST.Definition =>
        is(node, {
          type: 'definition',
        })
      ) ||
